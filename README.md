...
Next.js 16 の `proxy.ts`（middleware 後継）を使って、すべてのページに共通パスワード認証をかけています。  
利用する環境変数は次の 2 つです。

- `SITE_ACCESS_PASSWORD`: アクセス時に入力する共通パスワード
- `SITE_ACCESS_TOKEN`: 認証成功時に HttpOnly Cookie へ保存するランダムな固定トークン

`.env.local` の例:

```env
SITE_ACCESS_PASSWORD=your-password
SITE_ACCESS_TOKEN=replace-with-long-random-token
```
